"""
fillingsim_gpio_plc.py  –  Estación de llenado de botellas
...
- al terminar la animación -> zzz=1 con speed=20, good=2000, reject=12
...
def trigger_cycle():
    ...
    global fill_level, target_fill, status, color
    global good_count, bad_count

    activation_count += 1
    cycle_active   = True
    cycle_end_time = time.time() + CYCLE_DURATION_S
    fill_level     = GOOD_FILL
    target_fill    = GOOD_FILL
...
def stop_cycle():
    global cycle_active, target_fill, fill_level, status, color
...
def reset_all():
    ...
    global current_fill, target_fill, fill_level, cycle_active
    global status, color, toggle_states
...
pygame.display.set_caption("Bottle Fill Station + GPIO PLC Commands")
...
def get_bottle_points(x, y, width, height):
    ...
    points = [
        (x + width//2 - neck_width//2 - 5, y - 10),
        (x + width//2 + neck_width//2 + 5, y - 10),
        (x + width//2 + neck_width//2 + 5, y),
        (neck_x + neck_width, y),
        (neck_x + neck_width, body_y),
        (x + width, body_y),
        (x + width, body_y + body_height),
        (x, body_y + body_height),
        (x, body_y),
        (neck_x, body_y),
        (neck_x, y),
        (x + width//2 - neck_width//2 - 5, y),
    ]
    return points, body_y, body_height, neck_x, neck_width, neck_height

bottle_x, bottle_y, bottle_w, bottle_h = BOTTLE_W_AREA//2-60, 150, 120, 300
...
pygame.draw.lines(bottle_static, (200,200,200), True, bpts, 3)
for ml in [100,200,300,400,500]:
    my = int(body_y + body_h*(1-ml/510))
    pygame.draw.line(bottle_static, (150,150,150), (bottle_x-15,my), (bottle_x-5,my), 1)
    if ml % 200 == 0:
        bottle_static.blit(small_font.render(str(ml), True, (150,150,150)), (bottle_x-45,my-8))
...
try:
    while running:
        ...
        diff = target_fill - current_fill
        if abs(diff) > 0.5:
            current_fill += animation_speed * (1 if diff > 0 else -1)
            if diff > 0:
                current_fill = min(current_fill, target_fill)
            else:
                current_fill = max(current_fill, target_fill)
        ...
        screen.blit(small_font.render(
            f"Fill: {fill_level if fill_level is not None else '---'} ml", True, (255,255,255)), (20,25))
        screen.blit(small_font.render(
            f"Status: {status} | Ciclos: {activation_count}", True, (255,255,255)), (20,50))

        if fill_level is not None:
            bw = int((fill_level / 510.0) * 120)
            bc = (0,180,0) if fill_level >= 490 else (255,180,0) if fill_level >= 255 else (255,100,100)
            pygame.draw.rect(screen, (60,60,70), (250,35,120,15))
            if bw > 0:
                pygame.draw.rect(screen, bc, (250,35,bw,15))

        screen.blit(bottle_static, (0,0))

        if current_fill > 0:
            fill_pct = min(current_fill / MAX_ML, 1.0)
            total_h  = body_h + neck_h
            fill_h   = fill_pct * total_h
            liq_col  = (40,140,255)

            if fill_h <= body_h:
                pygame.draw.rect(screen, liq_col,
                    (bottle_x+3, body_y+(body_h-fill_h), bottle_w-6, fill_h))
            else:
                pygame.draw.rect(screen, liq_col, (bottle_x+3, body_y, bottle_w-6, body_h))
                nf = fill_h - body_h
                pygame.draw.rect(screen, liq_col, (neck_x+2, body_y-nf, neck_w-4, nf))

            if fill_h < total_h:
                sy = body_y+(body_h-fill_h) if fill_h <= body_h else body_y-(fill_h-body_h)
                sw = bottle_w-6 if fill_h <= body_h else neck_w-4
                sx = bottle_x+3 if fill_h <= body_h else neck_x+2
                pygame.draw.line(screen, (80,180,255), (sx,sy), (sx+sw,sy), 2)

        ind = (BOTTLE_W_AREA//2, bottle_y+bottle_h+55)
        pygame.draw.circle(screen, color, ind, 30)
        pygame.draw.circle(screen, (255,255,255), ind, 30, 2)
        s_short = "OK" if color == (0,200,0) else "DONE" if color == (0,150,255) else "FAIL" if color == (200,0,0) else "WAIT"
        ...
        screen.blit(label_font.render("⏱ T. EXECUTE", True, (0,210,80) if ea else (80,130,80)), (er.x+8,er.y+5))
        ...
        screen.blit(label_font.render("⏸ T. HELD", True, (255,160,0) if ha else (130,100,40)), (hr.x+8,hr.y+5))
